...
# Directory holding the heat-*.yaml templates this script renders. Set by
# roles/deploy-charts/tasks/nova_test.yaml, which stages them next to this
# script; there is no sensible default now that neither is in the checkout.
: ${HEAT_DIR:?HEAT_DIR must point at the directory holding heat-*.yaml}
: ${SSH_DIR:="${HOME}/.ssh"}

# Both directories are bind mounted at the same path inside the openstack
# client container, so `openstack stack create -t ${HEAT_DIR}/...` and
# `ssh -i ${SSH_DIR}/...` resolve there exactly as they do on the host.
OPENSTACK_CLIENT_CONTAINER_EXTRA_ARGS="${OPENSTACK_CLIENT_CONTAINER_EXTRA_ARGS:-} \
  -v ${HEAT_DIR}:${HEAT_DIR} -v ${SSH_DIR}:${SSH_DIR}"
